...
# AI usage: Used chatgpt for help debugging, writing difficult functions when stuck, to review code and give feedback
# Collaborators: Antonio Khemmoro
# Collaborated equally on this project. Each wrote two distinct calculations:

# Matthew's Functions: def calculate_avg_discount_by_region(superstore_data), def calculate_percent_discounted(superstore_data):
# Antiono's Functions: def calculate_avg_sales_by_category(superstore_data), def calculate_percent_profitable(superstore_data):

# Also, we both worked together on the shared components of the script:
# def_to_float to convert values
# read_csv_file(): for loading and preprocessing the data
# generate_report(): for formatting the output
# main(): for making the program execution

import csv

#Helper: converts values to float & blanks/NA as none
def _to_float(x):
    if x is None:
        return None
